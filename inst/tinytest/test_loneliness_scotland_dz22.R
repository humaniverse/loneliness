...
scotland_no_na <- na.omit(scotland_clinical_loneliness_dz)

# ---- Test DataZones ----
# Test class
expect_equal(
  class(scotland_no_na$dz11_code),
  "character"
)

# Test all DZ codes are present
expect_equal(
  sort(scotland_no_na$dz11_code),
...
# Test class
expect_equal(
  class(scotland_no_na$loneliness_zscore),
  "numeric"
)
...
# Test class
expect_equal(
  class(scotland_no_na$rank),
  "numeric"
)

# Compute ranks with same tie handling method to test actual ranks
expected_ranks <-
  rank(scotland_no_na$loneliness_zscore, ties.method = "average")
...
# Test class
expect_equal(
  class(scotland_no_na$deciles),
  "integer"
)

# Test number of values per bin
deciles <- scotland_no_na$deciles
...
actual_counts <- as.vector(table(scotland_no_na$deciles))
